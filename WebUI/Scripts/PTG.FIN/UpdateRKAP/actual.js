...
function upload() {
    ...
    url = urlAction("UpdateData", "Actual", "Upload");
...
function upload150() {
    var url = "";
    kendo.ui.progress($("#formWindow"), true);
    url = urlAction("UpdateData", "Actual150", "Upload");
    var doc = document.documentElement;
    var top = (window.pageYOffset || doc.scrollTop) - (doc.clientTop || 0);
    pos = -140 - top;
...
function download() {
    var url = urlAction("UpdateData", "Actual", "Download");
    var year = $("#Year").val();
    window.location.href = url + "?Year=" + year;
}
function download150() {
    var url = urlAction("UpdateData", "Actual150", "Download");
    var year = $("#Year").val();
    window.location.href = url + "?Year=" + year;
}
...
function setValidator() {
    $.validator.unobtrusive.parse("#actualForm");
    $("#actualForm").data("validator").settings.ignore = ":hidden:not(.validateAlways)";
...
function setValidator150() {
    $.validator.unobtrusive.parse("#actual150Form");
    $("#actual150Form").data("validator").settings.ignore = ":hidden:not(.validateAlways)";

    $("input").on("keyup blur", function () {
        if ($("#actual150Form").valid()) {
            $("button[type=submit]").prop("disabled", false);
        } else {
            $("button[type=submit]").prop("disabled", "disabled");
        }
...
function setFormUpload150() {
    $("#actual150Form").submit(function (e) {
        $("#YearReference").val($("#Year").val());
        if ($("input").hasClass("input-validation-error")) {
            e.preventDefault();
            return false;
